...
const databasePath = path.join(__dirname, "edulite.db");
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS assessments (
    ...
    type TEXT NOT NULL CHECK (
      type IN ('Major Exam', 'Activity', 'Quiz')
    ),
    date TEXT NOT NULL,
...
if (!columnExists("assessments", "created_at")) {
  db.exec(
...
if (!columnExists("assessment_scores", "created_at")) {
  db.exec(
...
if (!columnExists("assessment_scores", "updated_at")) {
  db.exec(
...
const migrateExistingData = db.transaction(() => {
  ...
  const unassignedAssessmentCount = db
    .prepare(`
  ...
  const unenrolledStudentCount = db
    .prepare(`
      SELECT COUNT(*) AS count
      FROM students
      WHERE NOT EXISTS (
        ...
        WHERE student_subjects.student_id = students.id
      )
    `)
    .get().count;
  ...
  if (tableExists("student_assessment_scores")) {
    const legacyCreatedAt = columnExists(
    ...
    const legacyUpdatedAt = columnExists(
      "student_assessment_scores",
      "updated_at",
    )
    ...
    db.prepare(`
      ...
      SELECT
        ...
        ${legacyCreatedAt},
        ${legacyUpdatedAt}
      FROM student_assessment_scores AS legacy
      WHERE NOT EXISTS (
        SELECT 1
        FROM assessment_scores AS current_score
        WHERE current_score.student_id = legacy.student_id
          AND current_score.assessment_id = legacy.assessment_id
      )
    `).run();
  }
...
db.exec(`
  CREATE UNIQUE INDEX IF NOT EXISTS idx_assessment_scores_unique
